    ...
    recognition.onresult = function (event) {
        ...
        Math.eval = function (command) {
            if (command.startsWith("sine ") || command.startsWith("cosecant inverse ")) {
                ...
                console.log(degree)
                result.value = command + " = " + Math.sin(degree * Math.PI / 180).toFixed(5);
                ...
                result.value = command + " = " + Math.cos(degree * Math.PI / 180).toFixed(5);
                ...
                result.value = command + " = " + Math.tan(degree * Math.PI / 180).toFixed(5);
                ...
                console.log(degree)
                result.value = command + " = " + (1 / Math.sin(degree * Math.PI / 180)).toFixed(5);
                ...
                result.value = command + " = " + (1 / Math.cos(degree * Math.PI / 180)).toFixed(5);
                ...
                result.value = command + " = " + (1 / Math.tan(degree * Math.PI / 180)).toFixed(5);
                ...
                if (!command.includes('inverse')) {
                    if (command.includes('base')) {
                        ...
                        result.value = command + " = " + (Math.log(array[2]) / Math.log(array[5])).toFixed(5);
                    } else {
                        const array = command.split(" ");
                        result.value = command + " = " + Math.log(array[2]).toFixed(5);
                    }
                } else {
                    //natural log inverse 3 with base 4 or natural log inverse 3
                    if (command.includes('base')) {
                        const array = command.split(" ");
                        result.value = command + " = " + (Math.log(array[6]) / Math.log(array[3])).toFixed(5);
                    } else {
                        const array1 = command.split(" ");
                        result.value = command + " = " + (1 / (Math.log(array1[3]))).toFixed(5);
                    }
                }
                ...
                if (!command.includes('inverse')) {
                    if (command.includes('base')) {
                        const array = command.split(" ");
                        result.value = command + " = " + (Math.log10(array[1]) / Math.log10(array[4])).toFixed(5);
                    } else {
                        const array = command.split(" ");
                        result.value = command + " = " + Math.log10(array[1]).toFixed(5);
                    }
                } else {
                    //log inverse 3 with base 4 or log inverse 3
                    if (command.includes('base')) {
                        const array = command.split(" ");
                        result.value = command + " = " + (Math.log10(array[5]) / Math.log10(array[2])).toFixed(5);
                    } else {
                        const array1 = command.split(" ");
                        result.value = command + " = " + (1 / (Math.log10(array1[2]))).toFixed(5);
                    }
                }
            } else if (command.startsWith("exponential ") && command.includes(" ^ ")) {
                // exponential to the power 3
                const val = command.split(" ");
                result.value = command + " = " + Math.exp(val[2]).toFixed(5);
            } else if (command.includes(" ^ ")) {
                const val = command.split(" ");
                result.value = command + " = " + Math.pow(val[0], val[2]).toFixed(5);
            } else if (command.includes("pi")) {
                // pi function - 7 pi
                const val = command.split(" ");
                result.value = command + " = " + val[0] * Math.PI.toFixed(5);
                ...
                result.value = command + " = " + final;
            } else if (command.includes("root")) {
                console.log('inside root')
                if (command.startsWith("square root of ")) {
                    const val = command.split(" ");
                    result.value = command + " = " + Math.sqrt(val[val.length - 1]).toFixed(5);
                } else if (command.startsWith("cube root of ")) {
                    const val = command.split(" ");
                    result.value = command + " = " + Math.cbrt(val[val.length - 1]).toFixed(5);
                } else {
                    // fourth root of 3
                    ...
                    console.log(root)
                    result.value = command + " = " + Math.pow(val[val.length - 1], (1 / root)).toFixed(5);
                }
            } else {
                for (var i = 0; i < command.length; i++) {
                    ...
                    if (isNaN(command[i]) && !['+', '-', '/', '*', '%'].includes(command[i])) {
                        ...
                        result.value = command + " = " + eval(command);
                    }
                }
            }
        }
        ...
        setTimeout(function () {
            ...
            synth.speak(toSpeak);
            sessionStorage.setItem('expression', document.getElementById("result").value);
